...
function processPoolRemoved(snapshot, state) {
  ...
  db.ref(state.network + "/stake_pools")
    .child(snapshot.key)
    .child("d")
    .get()
    .then((val) => {
      ...
      if (update) {
        if (Object.keys(state.poolindex).indexOf(snapshot.key) == "-1") {
          ...
          Vue.set(
            state.pools,
            state.poolindex[snapshot.key],
            convertPool(pooldata, state)
          );
        }
      }
    });
...
function processPool(snapshot, state) {
  if (Object.keys(state.poolindex).indexOf(snapshot.key) == "-1") {
    ...
    Vue.set(
      state.pools,
      state.poolindex[snapshot.key],
      convertPool(snapshot.val(), state)
    );
  }
}
...
function convertPool(a, state) {
  ...
  if (
    ...
    b.assigned_slots_epoch >= state.sync_data.currentepoch - 1
  ) {
...
export const store = new Vuex.Store({
  ...
  actions: {
    ...
    bindUserData: firebaseAction(({ state, bindFirebaseRef }) => {
      bindFirebaseRef(
        "userData",
        db.ref(state.network + "/users/privMeta/" + state.userId)
      );
      bindFirebaseRef(
        "userDataPub",
        db.ref(state.network + "/users/pubMeta/" + state.userId)
      );
      bindFirebaseRef(
        "userDataPriv",
        db.ref(state.network + "/users/privMeta/" + state.userId)
    ...
    bindPoolsRetired: firebaseAction(({ state, unbindFirebaseRef }) => {
      if (!state.retiredPoolsSubscribed) {
        const poolsRef = db
          .ref(state.network + "/stake_pools")
          .orderByChild("d")
          .equalTo(true);
        poolsRef.on("child_added", (snapshot) => {
        ...
        poolsRef.on("child_changed", (snapshot) => {
          processPool(snapshot, state);
        });
        poolsRef.on("child_moved", (snapshot) => {
          processPool(snapshot, state);
        });
        poolsRef.on("child_removed", (snapshot) => {
    ...
    bindSinglePool: firebaseAction(({ state, bindFirebaseRef }, pool_id) => {
      if (Object.keys(state.poolindex).indexOf(pool_id) == "-1") {
        if (typeof state.singlepoolssub[pool_id] == "undefined") {
          ...
          bindFirebaseRef(
            livedata,
            db.ref(state.network + "/stake_pools").child(pool_id),
            {
              serialize: (snapshot) => {
                ...
                if (pooldata != null) {
                  if (Object.keys(state.poolindex).indexOf(pool_id) == "-1") {
                    const daindex =
                      state.pools.push(convertPool(pooldata, state)) - 1;
                    Vue.set(state.poolindex, pool_id, daindex);
                    Vue.set(state.pools, daindex, convertPool(pooldata, state));
                  } else {
                    Vue.set(
                      state.pools,
                      state.poolindex[pool_id],
                      convertPool(pooldata, state)
                    );
                  }
                }
    ...
    bindGenesis: firebaseAction(({ state, bindFirebaseRef }) => {
      //, unbindFirebaseRef
      bindFirebaseRef("promotions", db.ref(state.network + "/promotions"));
      bindFirebaseRef(
        "heights",
        db.ref(state.network + "/stake_pool_columns/heights")
      );
      //bindFirebaseRef ('livestake',db.ref(state.network+'/stake_pool_columns/livestake'))
      bindFirebaseRef(
        "mary_db_sync_status_pool_forecast_calculated_epoch",
        db.ref(
          state.network + "/mary_db_sync_status/pool_forecast_calculated_epoch"
        )
      );
      bindFirebaseRef(
        "mary_db_sync_status_pool_actuals_calculated_epoch",
        db.ref(
          state.network + "/mary_db_sync_status/pool_actuals_calculated_epoch"
        )
      );

      bindFirebaseRef(
        "mary_db_sync_status_forecast_rewards_complete_epoch",
        db.ref(
          state.network + "/mary_db_sync_status/forecast_rewards_complete_epoch"
        )
      );
      bindFirebaseRef(
        "mary_db_sync_status_actual_rewards_complete_epoch",
        db.ref(
          state.network + "/mary_db_sync_status/actual_rewards_complete_epoch"
        )
      );
      bindFirebaseRef(
        "mary_db_sync_status_new_rewards_complete_epoch",
        db.ref(
      ...
      bindFirebaseRef(
        "admin_message",
        db.ref(state.network + "/admin_message/web")
      );
      bindFirebaseRef(
        "quietEpoch",
        db.ref(state.network + "/recent_block/epoch")
      );
      bindFirebaseRef("langs_raw", db.ref(state.network + "/languages"));

      bindFirebaseRef(
        "most_recent_block",
        db.ref(state.network + "/recent_block")
      ).then(() => {
        bindFirebaseRef(
          "livedata_old",
          db.ref(
            state.network +
              "/epoch_params/" +
              (state.most_recent_block.epoch - 1)
          )
        );
        bindFirebaseRef(
          "livedata",
          db.ref(
            state.network + "/epoch_params/" + state.most_recent_block.epoch
          )
        );
        bindFirebaseRef(
          "epoch_data",
          db.ref(state.network + "/epochs/" + state.most_recent_block.epoch)
        );

        bindFirebaseRef(
          "active_stake",
          db
            .ref(state.network + "/total_active_stake")
            .child(state.most_recent_block.epoch)
        );
      });
  ...
  getters: {
    ...
    getGenesis: (state) => {
      return {
        blockstake:
          state.active_stake != null ? state.active_stake[".value"] : 0,
        ...
        activeSlotCoeff:
          state.ecosystem != null ? state.ecosystem.activeSlotCoeff : 0.05,
        slot_in_epoch:
        ...
        forecast_rewards_complete_epoch:
          state.mary_db_sync_status_forecast_rewards_complete_epoch != null
            ? state.mary_db_sync_status_forecast_rewards_complete_epoch[
                ".value"
              ]
            : 0,
        actual_rewards_complete_epoch:
          state.mary_db_sync_status_actual_rewards_complete_epoch != null
            ? state.mary_db_sync_status_actual_rewards_complete_epoch[".value"]
            : 0,
        ...
        pool_forecast_calculated_epoch:
          state.mary_db_sync_status_pool_forecast_calculated_epoch != null
            ? state.mary_db_sync_status_pool_forecast_calculated_epoch[".value"]
            : 0,
        pool_actuals_calculated_epoch:
          state.mary_db_sync_status_pool_actuals_calculated_epoch != null
        ...
        livedata2: {
          ...
          total_blockstake:
            state.active_stake != null ? state.active_stake[".value"] : 0,
          active_pools:
            state.ecosystem != null ? state.ecosystem.activePools : 0,
          total_epoch_blocks:
        ...
        livedata1: {
          decentralisationParam:
            state.livedata != null ? state.livedata.decentralisation : 1,
          reserves: state.ecosystem != null ? state.ecosystem.reserves : 0,
